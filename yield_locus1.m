...
%             Wmax=0;
        ...
        s_x_mean(u,1)= mean(s_g_x(:,1));
        s_y_mean(u,1)= mean(s_g_y(:,1));
%         s_x_mean(u,2)= mean(s_g_x(:,2));
%         s_y_mean(u,2)= mean(s_g_y(:,2));
        
    end 
%z=zeros(p-1,p-1);
% plot(s_x_mean,s_y_mean,'r+')
plot(s_x_mean(:,1),s_y_mean(:,1),'r+')
%hold on
%plot(s_x_mean(:,2),s_y_mean(:,2),'b+')
%hold off
%fill(s_g_x,s_g_y,'r')
%surf(s_g_x,s_g_y,z)
